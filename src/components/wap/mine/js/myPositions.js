    ...
    myPositions(){
      ...
      this.$api.fetchPost('/sanic-api', dt).then(r=>{
        if(r.user_invest_account_by_uid!=undefined && r.user_invest_account_by_uid.length>0){
          ...
          this.myInfo["all_amt"] = (init_amt+hold_profit+day_profit).toFixed(2)
          this.myInfo["all_profit"] = (hold_profit+day_profit).toFixed(2)
          this.myInfo["now"] = this.$utdate.dateFmt(new Date(),"MM月dd日")      
          // for(var x of lst){
          //   x.inserttime = this.$utdate.dateFmt(new Date(),"MM月dd日")
          ...
          day_profit = eval(this.myTargets.map(x=>x.daily_profit).join("+"))
          init_amt = eval(this.myTargets.map(x=>x.init_amt).join("+"))
          let t_amt = init_amt+hold_profit+day_profit
          ...
          t_amt = init_amt+hold_profit+day_profit
