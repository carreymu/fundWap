          ...
          <flexbox-item>
            <div class="itemContent" v-for="(item,index) in it.funds" :key="index">
                <flexbox orient="vertical">
                  ...
                  <flexbox-item class="cont">
                    <x-button style="width:70%;height:40px;font-size:15px;" type="warn" action-type='button' 
                    :link="'/fundWap/bestChoiceDetail/'+item.id" v-show="item.status==1">{{item.minInvest}}起投</x-button>
                    <x-button style="width:70%;height:40px;font-size:15px;" type="default" v-show="item.status==2" action-type='button' >已售完</x-button>
                  </flexbox-item>
                </flexbox>
              </div>
            </flexbox-item>
